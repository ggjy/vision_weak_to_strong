...
__all__ = ['ResNet', 'resnet20', 'resnet32', 'resnet56', 'resnet110', 'resnet8x4', 'resnet32x4']
...
class ResNet(nn.Module):
    def __init__(self, depth, num_filters, block_name="BasicBlock", num_classes=10, **kwargs):
    ...
    def forward(self, x, requires_feat=False):
        ...
        feat = [f0, f1, f2, f3, avg]

        return (out, feat) if requires_feat else out
    
    def stage_info(self, stage):
        if stage == 1:
            index = 0
            shape = (self.stage_channels[0], 32, 32)
        elif stage == 2:
            index = 1
            shape = (self.stage_channels[1], 32, 32)
        elif stage == 3:
            index = 2
            shape = (self.stage_channels[2], 16, 16)
        elif stage == 4:
            index = 3
            shape = (self.stage_channels[3], 8, 8)
        elif stage == -1:
            index = -1
            shape = self.stage_channels[-1]
        else:
...
@register_model
def resnet80(**kwargs):
    return ResNet(80, [16, 16, 32, 64], "basicblock", **kwargs)
