...
def train_sel(args, scheduler, model, model_ema, contrast, queue, device, train_loader, train_selected_loader,
              optimizer, epoch, selected_pairs):
    ...
    for batch_idx, (img, labels, index) in enumerate(train_loader):
        ...
        if args.sup_queue_use == 1:
            queue.enqueue_dequeue(torch.cat((embedA_ema.detach(), embedB_ema.detach()), dim=0),
                                  torch.cat((predsA_ema.detach(), predsB_ema.detach()), dim=0),
                                  torch.cat((index.detach().squeeze(), index.detach().squeeze()), dim=0))
        ...
        maskUnsup_batch, maskUnsup_mem, mask2Unsup_batch, mask2Unsup_mem = unsupervised_masks_estimation(args, queue,
                                                                                                         mix_index1,
                                                                                                         mix_index2,
                                                                                                         epoch, bsz,
                                                                                                         device)
        ...
        maskSup_batch, maskSup_mem, mask2Sup_batch, mask2Sup_mem = \
            supervised_masks_estimation(args, index.long(), queue, queue_index.long(), mix_index1, mix_index2, epoch,
        ...
        loss_sup = Supervised_ContrastiveLearning_loss(args, pairwise_comp_batch, maskSup_batch, mask2Sup_batch,
                                                       maskUnsup_batch, mask2Unsup_batch, logits_mask_batch, lam1, lam2,
                                                       bsz, epoch, device, batch_idx)
        
        ## compute simi_loss
        loss_simi = Simi_loss(args, pros_simi_batch, maskSup_batch, mask2Sup_batch, maskUnsup_batch, mask2Unsup_batch,
                              logits_mask_batch, lam1, lam2, bsz, epoch, device, batch_idx)
        ...
        if args.sup_queue_use == 1 and epoch > args.sup_queue_begin:
            ...
            loss_mem = Supervised_ContrastiveLearning_loss(args, pairwise_comp_mem, maskSup_mem, mask2Sup_mem,
                                                           maskUnsup_mem, mask2Unsup_mem, logits_mask_mem, lam1, lam2,
                                                           bsz, epoch, device, batch_idx)
            
            loss_sup = loss_sup + loss_mem
            
            loss_simi_mem = Simi_loss(args, pros_simi_mem, maskSup_mem, mask2Sup_mem, maskUnsup_mem, mask2Unsup_mem,
                                      logits_mask_mem, lam1, lam2, bsz, epoch, device, batch_idx)
...
def train_sup(args, scheduler, model, model_ema, contrast, queue, device, train_loader, train_selected_loader,
              optimizer, epoch, noisy_pairs):
    train_loss_1 = AverageMeter()
    train_loss_3 = AverageMeter()
...
def pair_selection(args, net, device, trainloader, testloader, epoch):
    ...
    with torch.no_grad():
        ...
        for batch_idx, (inputs, targets, index) in enumerate(temploader):
            ...
            yd, yi = dist.topk(args.k_val, dim=1, largest=True,
            ...
            retrieval = torch.gather(candidates, 1,
                                     yi)  ## From replicated labels get those of the top-K neighbours using the index yi (from top-k operation)
    ...
    with torch.no_grad():
        ...
        noisy_pairs[
            index_selected.unsqueeze(1).expand(total_selected_num, total_selected_num), index_selected.unsqueeze(
                    0).expand(total_selected_num, total_selected_num)] = selected_pairs
